...
export async function sendMail({ to, cc, subject, text, html, smtpConfig = null }) {
  ...
  const headerValues = [to, ...(Array.isArray(cc) ? cc : [cc]), subject, cfg.host, cfg.user, cfg.fromName, cfg.fromAddress].filter(Boolean);
  ...
  await transporter.sendMail({ from, to, cc, subject, text, html, disableFileAccess: true, disableUrlAccess: true });
  return { sent: true };
}
